...
class MatchGame extends Component {
  ...
  getRandomImage = arr => {
    ...
    const selectedArray = arr[randomIndex]
    const {id} = selectedArray
    this.setState({randomImageId: id})
  }
  ...
  onImageSelect = id => {
    ...
    if (id === randomImageId) {
      ...
      this.getRandomImage(imagesList)
    } else {
      this.setState({isGameOn: false})
      clearInterval(this.timerId)
    }
  ...
  render() {
    ...
    return (
      <div className="container">
        <Navbar score={score} remainingTime={remainingTime} />
        <div className="sub-container">
          ...
          {!isGameOn && <GameOver onReplay={this.onReplay} score={score} />}
        </div>
      </div>
    )
  }
